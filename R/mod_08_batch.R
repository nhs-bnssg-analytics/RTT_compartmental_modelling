  ...
  moduleServer(id, function(input, output, session){
    ...
    observeEvent(
      ...
      c(input$batch_run_rtt_data), {
        ...
        # dummy trust names instead of input$trust_codes
        dummy_trust_names <- c(
        ...
        raw_data <- seq(
          from = lubridate::floor_date(
            .data$min_download_date, unit = "months"
          ),
          to = lubridate::floor_date(
            .data$max_download_date, unit = "months"
          ),
          by = "months"
        ) |>
          purrr::map(
        ...
        forecast_months <- lubridate::interval(
          as.Date(max_download_date %m+% months(1)),
        ...
        projection_referrals <- raw_data |>
          ...
          tidyr::complete(
            ...
            period_id = unique(raw_data$period_id),
        ...
        incompletes_at_t0 <- raw_data |>
          ...
          tidyr::complete(
            specialty = selections_labels$specialties$selected_name,
            months_waited_id = unique(raw_data$months_waited_id),
            trust = selections_labels$trusts$selected_name,
            fill = list(value = 0)
          ) |>
          rename(
        ...
        period_lkp <- dplyr::tibble(
          period_id = seq_len(max(raw_data$period_id) + forecast_months + 1),
          period = seq(
