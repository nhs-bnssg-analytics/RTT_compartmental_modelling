...
if (params$method == "mean") {
  method_text <- "an average"
} else if (params$method == "median") {
  method_text <- "the median"
} else if (params$method == "latest") {
  method_text <- "the latest"
}

if (params$renege_rate == "historic") {
  rr_text <- "the median proportion observed for each trust-specialty combination over the last 12 months"
} else {
  rr_text <- paste0("the user defined proportion of ", params$renege_rate, "%")
}
```
...
3. The proportion of departures that are reneges is equal to (or as close to) `r rr_text`.

The profile of how treatments are distributed across the waiting list is based on the treatment distribution observed over the last 12 months, taking `r method_text` distribution for the period.
    ...
    dplyr::rename(
      dplyr::any_of(
        c(
          ...
          # "Steady state activity to first attendance (outpatient)*" = "op_to_first",
          # "Steady state follow up attendances (outpatient)*" = "op_follow_up",
          # "Steady state inpatient activity (day cases)*" = "ip_day",
          # "Steady state inpatient activity (non-day cases)*" = "ip_non_day",
  ...
  dplyr::rename(
    dplyr::any_of(
      c(
        ...
        "Steady state_Crude activity to first attendance (outpatient)" = "op_to_first",
        "Steady state_Crude follow up attendances (outpatient)" = "op_follow_up",
        "Steady state_Crude inpatient activity (day cases)" = "ip_day",
        "Steady state_Crude inpatient activity (non-day cases)" = "ip_non_day",
        "Current / steady state waiting list size" = "current_vs_ss_wl_ratio",
        "Additional monthly removals required" = "monthly_removals"
      )
    )
  ) |> 
  mutate(`Trust` = gsub(" NHS| FOUNDATION| TRUST| HEALTHCARE", "", `Trust`),
    `Trust` = tools::toTitleCase(tolower(`Trust`))) |> 
...
for (i in seq_len(length((split_tbl)))) {
  ...
  out <- split_tbl[[i]] |> 
    ...
    flextable::width(width = 1.25, unit = "cm") |>
    flextable::theme_vanilla() |>
    flextable::bg(part = "header", bg = "#002060") |> 
    flextable::color(part = "header", color = "white") |>
    flextable::bold(part = "header", bold = TRUE) |> 
    flextable::bg(i = seq(1, nrow(split_tbl[[i]]), 2), bg = "#CCD2D8")

  if (nrow(split_tbl[[i]]) > 1) {
    out <- out |>  
