...
plot_output(
  ...
  p_perc = F,
  data_source = params$chart_specification$data_source
)
...
```{r}
#| label: clock-stop-calcs
#| results: 'asis'
#| ft.align: 'left'

clock_stops <- params$waiting_list |>
...
activity_table <- setNames(
    ...
    nm = params$chart_specification$specialty
  ) |>
...
if (!"Unable to compute" %in% names(activity_table)) {
  ...
  activity_table |> 
    ...
    flextable::set_formatter(
      proportion = function(x) sprintf("%.1f%%", x * 100)
    ) |> 
    flextable::set_header_labels(
      ...
      count = "Count",
      proportion = "Proportion of pathway type"
      ) |> 
    flextable::flextable_to_rmd()
}
```
